...
export default function App() {
  ...
  const [editExperience, setEditExperience] = useState<string>('0');
  ...
  const calculateProgress = () => {
    let total = 1; // Solo nombre para usuarios básicos
    let completed = editName.trim() ? 1 : 0;

    if (isEditWorker) {
      // Campos extra para trabajadores
      const workerFields = [
        editPhone.trim() !== '',
        editDept !== '',
        editCity !== '',
        editOccupations.length > 0,
        editExperience !== '' && !isNaN(parseInt(editExperience)),
        editDescription.trim() !== ''
      ];
      total += workerFields.length;
      completed += workerFields.filter(Boolean).length;
    }
  ...
  useEffect(() => {
    if (profile && view === 'EDIT_PROFILE') {
      ...
      setIsEditWorker(profile.isWorker || false);
      setEditExperience(profile.experience?.toString() || '0');
      setEditDescription(profile.description || '');
      setEditOccupations(profile.occupations || []);
      setFormErrors([]);
    }
  }, [profile, view]);
      ...
      <main className="pt-16 min-h-[calc(100vh-64px)] overflow-x-hidden">
        <AnimatePresence mode="wait">
            ...
            >
              {profile?.isWorker ? (
                <div className="flex flex-col items-center justify-center py-24 text-center">
                  ...
                  <h3 className="text-2xl font-display font-bold text-brand-primary">Acceso Restringido</h3>
                  <p className="text-slate-500 mt-2 max-w-sm">
                    Como estás registrado como <strong>Trabajador</strong>, no puedes buscar a otros profesionales. 
                    Esta función está reservada para que los clientes encuentren tus servicios.
                  </p>
                  <button 
                    onClick={() => setView('HOME')}
                    className="mt-6 bg-brand-cta text-white px-8 py-3 rounded-xl font-bold hover:bg-blue-700 transition-all shadow-lg active:scale-95"
                  >
                    Volver al Inicio
                  </button>
                </div>
          ...
          {view === 'PROFILE' && selectedWorker && (
            ...
            >
              {profile?.isWorker ? (
                <div className="flex flex-col items-center justify-center py-24 text-center bg-white rounded-[2rem] shadow-xl border border-slate-100">
                  <div className="w-20 h-20 bg-slate-100 rounded-full flex items-center justify-center text-slate-400 mb-6">
                    <UserIcon size={40} />
                  </div>
                  <h3 className="text-2xl font-display font-bold text-brand-primary">Acceso no permitido</h3>
                  <p className="text-slate-500 mt-2 max-w-sm">
                    Para visualizar el perfil de otros trabajadores, debes tener una cuenta de cliente.
                  </p>
                  <button 
                    onClick={() => setView('HOME')}
                    className="mt-6 text-brand-cta font-bold hover:underline"
            ...
            >
              <div className="mb-12 space-y-4">
                ...
                <div className="space-y-2">
                  <div className="flex justify-between items-end">
                    ...
                    <span className={cn(
                      "text-sm font-bold",
                      calculateProgress() === 100 ? "text-emerald-500" : "text-brand-cta"
                    )}>
                      {calculateProgress()}%
                    </span>
                  </div>
                  <div className="h-2 bg-slate-100 rounded-full overflow-hidden border border-slate-200/50">
                    <motion.div 
                      ...
                      className={cn(
                        ...
                        calculateProgress() === 100 ? "bg-emerald-500" : "bg-brand-cta"
                      )}
                    />
                  </div>
                  <p className="text-[11px] text-slate-400 italic">
                    {calculateProgress() < 100 
                      ? "Completá los campos marcados con (*) para llegar al 100%."
                      : "¡Genial! Tu perfil está completo y listo para destacar."
                    }
                  </p>
                </div>
              </div>
              ...
              <form 
                onSubmit={(e) => {
                  ...
                  if (!editName.trim()) errors.push("El nombre es obligatorio.");

                  if (isEditWorker) {
                    if (!editPhone.trim()) errors.push("El teléfono es obligatorio para trabajadores.");
                    if (!editDept) errors.push("El departamento es obligatorio.");
                    if (!editCity) errors.push("La localidad es obligatoria.");
                    if (editOccupations.length === 0) errors.push("Debes seleccionar al menos un oficio.");
                    if (editExperience === '' || isNaN(parseInt(editExperience))) errors.push("Los años de experiencia son obligatorios.");
                    if (!editDescription.trim()) errors.push("La descripción de servicios es obligatoria.");
                  }
                  ...
                  handleUpdateProfile({
                    ...
                    priceRange: (e.currentTarget.elements.namedItem('priceRange') as HTMLSelectElement).value as any,
                  });
                }}
                className="space-y-8"
              >
                {formErrors.length > 0 && (
                  ...
                  >
                    <p className="font-bold mb-2">Por favor, corregí los siguientes errores:</p>
                ...
                <div className="bg-white p-8 rounded-3xl border border-slate-100 shadow-xl shadow-slate-200/40 space-y-6">
                  <div className="grid sm:grid-cols-2 gap-6">
                    ...
                    <div className="space-y-2">
                      <label className="text-xs font-bold text-slate-400 uppercase tracking-widest">
                        Teléfono de Contacto {isEditWorker && <span className="text-red-500">*</span>}
                      </label>
                      <input 
                        name="phone"
                        value={editPhone}
                  ...
                  <div className="flex items-center gap-3 p-4 bg-slate-50 rounded-2xl border border-slate-100">
                    <input 
                      type="checkbox"
                      name="isWorker"
                      checked={isEditWorker}
                      onChange={(e) => setIsEditWorker(e.target.checked)}
                      id="isWorker"
                      className="w-5 h-5 rounded border-slate-300 text-brand-cta focus:ring-brand-cta cursor-pointer"
                    />
                    <label htmlFor="isWorker" className="font-bold text-brand-primary cursor-pointer">Quiero ofrecer mis servicios (Perfil Trabajador)</label>
                  </div>

                  <div className="grid sm:grid-cols-2 gap-6">
                    <div className="space-y-2">
                      <label className="text-xs font-bold text-slate-400 uppercase tracking-widest">
                        Departamento {isEditWorker && <span className="text-red-500">*</span>}
                      </label>
                      <select 
                        name="department"
                        value={editDept}
                    ...
                    <div className="space-y-2">
                      <label className="text-xs font-bold text-slate-400 uppercase tracking-widest">
                        Localidad {isEditWorker && <span className="text-red-500">*</span>}
                      </label>
                      <select 
                        name="city"
                        value={editCity}
                  ...
                  <div className="space-y-2">
                    <label className="text-xs font-bold text-slate-400 uppercase tracking-widest">
                      Oficios {isEditWorker && <span className="text-red-500">*</span>} (Mantené Ctrl para seleccionar varios)
                    </label>
                    <select 
                      name="occupations"
                      multiple
                      value={editOccupations}
                      onChange={(e) => {
                  ...
                  <div className="grid sm:grid-cols-2 gap-6">
                    <div className="space-y-2">
                      <label className="text-xs font-bold text-slate-400 uppercase tracking-widest">
                        Años de Experiencia {isEditWorker && <span className="text-red-500">*</span>}
                      </label>
                      <input 
                        type="number"
                        name="experience"
                  ...
                  <div className="space-y-2">
                    <label className="text-xs font-bold text-slate-400 uppercase tracking-widest">
                      Descripción de Servicios {isEditWorker && <span className="text-red-500">*</span>}
                    </label>
                    <textarea 
                      name="description"
